# Component Quality Reports

This directory contains detailed quality reports for each frontend component.

## Appointment Module

- [AppointmentRescheduleComponent](./appointment-reschedule.md)
- [AppointmentListComponent](./appointment-list.md)
- [AppointmentService](./appointment-service.md)
- [AppointmentDetailComponent](./appointment-detail.md)

## Dashboard Module

- [DashboardComponent](./dashboard.md)
- [StatsWidgetComponent](./stats-widget.md)
- [RevenueChartComponent](./revenue-chart.md)

## Auth Module

- [LoginComponent](./auth-login.md)
- [ProfileComponent](./profile.md)

## Salon Module

- [SalonProfileComponent](./salon-profile.md)

## Settings

- [SettingsComponent](./settings.md)

## Navigation

- [Back to Quality Standards](../QUALITY_STANDARDS.md)
